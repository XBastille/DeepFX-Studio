...
class TextDataBunch(DataBunch):
    ...
    def load(
        ...
        **kwargs,
        ...
        train_ids, train_lbls = np.load(cache_path / f"train_ids.npy"), np.load(
            cache_path / f"train_lbl.npy"
        )
        valid_ids, valid_lbls = np.load(cache_path / f"valid_ids.npy"), np.load(
...
class SPProcessor(PreProcessor):
    ...
    def process(self, ds):
        ...
        if not getattr(self, "vocab", False):
            with open(self.sp_vocab, "r") as f:
