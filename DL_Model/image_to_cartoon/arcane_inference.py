...
def analyze_facial_features(image_input):
    detected_boxes, detected_prob, detected_landmarks = mtcnn.detect(image_input, landmarks=True)
    if not mtcnn.keep_all:
        detected_boxes, detected_prob, detected_landmarks = mtcnn.select_boxes(
            detected_boxes, detected_prob, detected_landmarks, 
            image_input, method=mtcnn.selection_method
        )
...
def calculate_image_dimensions(boxes, image, max_res=1_500_000, 
                             face_target=256, custom_ratio=0, scale_limit=2):
...
def optimize_image_size(input_image, max_res=1_500_000, 
                       face_target=256, custom_ratio=0, scale_limit=2):
    detected_boxes, _ = analyze_facial_features(input_image)
    return calculate_image_dimensions(detected_boxes, input_image, 
                                    max_res, face_target, 
                                    custom_ratio, scale_limit)
...
def transform_tensor_to_img(tensor_data):
    return tensor_data.mul(t_stds).add(t_means).mul(255.).clamp(0,255).permute(1,2,0)
...
def execute_transformation(input_path, model_path):
    ...
    optimized_image = optimize_image_size(source_image, face_target=300, 
                                        max_res=1_500_000, scale_limit=2)
