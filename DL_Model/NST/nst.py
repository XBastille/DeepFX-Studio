...
class NeuralStyleTransfer:
    ...
    def load_and_preprocess_images(self, content_image_path, style_image_path):
        self.content_image = tf.constant(np.reshape(
            np.array(Image.open(content_image_path).resize((self.img_size, self.img_size))), ((1,) + (self.img_size, self.img_size, 3))
        ))
        self.style_image = tf.constant(np.reshape(
            np.array(Image.open(style_image_path).resize((self.img_size, self.img_size))), ((1,) + (self.img_size, self.img_size, 3))
        ))
        ...
        self.generated_image = tf.clip_by_value(self.generated_image, clip_value_min=0.0, clip_value_max=1.0)
    ...
    def set_content_and_style_targets(self):
        ...
        self.a_C = vgg_model_outputs(tf.Variable(tf.image.convert_image_dtype(self.content_image, tf.float32)))
        self.a_S = vgg_model_outputs(tf.Variable(tf.image.convert_image_dtype(self.style_image, tf.float32)))
