...
class RealESRGAN:
    ...
    def upscale_img(self, input_img, batch_size=4, patch_size=192,
                     overlap=24, padding=15):
        scale = self.scale
        device = self.device
        ...
        tensor_input = torch.FloatTensor(patches/255).permute((0,3,1,2)).to(device).detach()
        ...
        with torch.no_grad():
            ...
            for idx in range(batch_size, tensor_input.shape[0], batch_size):
                processed_batch = torch.cat((processed_batch, 
                                          self.model(tensor_input[idx:idx+batch_size])), 0)
