...
class VideoColorizer:
    ...
    def _extract_raw_frames(self, source_path: Path):
        ...
        process = (
            ...
            .output(
                bwframe_path_template, format="image2", vcodec="mjpeg", **{"q:v": "0"}
            )
    ...
    def _build_video(self, source_path: Path) -> Path:
        ...
        os.system(
            f'ffmpeg -y -i "{str(source_path)}" -vn -acodec copy "{str(audio_file)}" -hide_banner -nostats -loglevel error'
        )

        if audio_file.exists():
            os.system(
                f'ffmpeg -y -i "{str(colorized_path)}" -i "{str(audio_file)}" -shortest -c:v copy -c:a aac -b:a 256k "{str(result_path)}" -hide_banner -nostats -loglevel error'
            )
        logging.info(f"Video created here: {str(result_path)}")
        return result_path
