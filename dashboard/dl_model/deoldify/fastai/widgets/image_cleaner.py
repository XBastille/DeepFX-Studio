...
class ImageCleaner:
    ...
    def make_vertical_box(cls, children, layout=Layout(), duplicates=False):
        ...
        if not duplicates:
            ...
            return widgets.VBox([children[0], children[2]], layout=layout)
    ...
    def create_image_list(self, dataset, fns_idxs):
        ...
        if self._duplicates:
            ...
            chunked_idxs = [
                chunk
                for chunk in chunked_idxs
                if Path(items[chunk[0]]).is_file() and Path(items[chunk[1]]).is_file()
            ]
            return [
                (dataset.x[i]._repr_jpeg_(), items[i], self._labels[dataset.y[i].data])
                for chunk in chunked_idxs
                for i in chunk
            ]
        else:
