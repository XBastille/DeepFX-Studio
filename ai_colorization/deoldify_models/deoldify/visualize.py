...
from .filters import ImgFilterInterface, MultiStageFilter, ColorizeFilter
...
class ModelImageEnhancer:
    ...
    def bnw_img(
        ...
        bnwd = self.generate_bnwd_img(
            path, quality_factor, post_process=post_process
        )
        original = self._load_img(path)

        if compare:
            self._display_comparison(figsize, quality_factor, display_quality, original, bnwd)
        else:
    ...
    def _display_comparison(
        ...
        self._display_img(original, axes=axes[0], figsize=figsize, 
                          quality_factor=quality_factor, display_quality=False)
        self._display_img(bnwd, axes=axes[1], figsize=figsize, 
                          quality_factor=quality_factor, display_quality=display_quality)
    ...
    def _display_single(  
        ...
        self._display_img(bnwd, axes=axes, figsize=figsize, 
                          quality_factor=quality_factor, display_quality=display_quality)
    ...
    def generate_bnwd_img(
        self, path: Path, quality_factor: int = None, post_process: bool = True,
    ) -> Image:
    ...
    def _display_img(
        ...
        if quality_factor is not None and display_quality:
            plt.text(10, 10, f'quality_factor: {quality_factor}',
...
def create_stable_colorizer(
    ...
    filter = MultiStageFilter([ColorizeFilter(learner=learn)], quality_factor=quality_factor)
